...
def markdown_report(data: dict[str, Any]) -> str:
    ...
    lines.extend([
        "", "## 扫描边界", "",
        "- 未读取 `.env`、token、密钥、命令历史和 MCP 配置值。",
        "- 默认不执行任何发现到的工具；只有加 `--probe-versions` 才执行安全版本探测。",
        "- 未发现不等于不存在，可能位于自定义目录，可用 `--skill-root` 或 `--cli` 补充。",
        "- 真实任务验收仍需完成：已发现 → 已认证 → 能执行 → 能回读。",
        "",
    ])
    return "\n".join(lines)
...
def main() -> int:
    ...
    clis, missing = discover_clis(DEFAULT_CLIS + args.cli, home, args.probe_versions)
    ...
    data: dict[str, Any] = {
        ...
        "skills": skills,
        "clis": clis,
        "missing_cli_candidates": missing,
        "connectors": connectors,
    }

    json_path = output_dir / "capability-map.json"
    md_path = output_dir / "capability-map.md"
    ...
    print(json.dumps({
        ...
        "routes": sum(1 for route in routes if route["status"] == "discovered"),
